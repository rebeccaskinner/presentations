...
clearTerm, reverseVideo, resetVideo :: Text
clearTerm =  "\^[[1J\^[[1;1H"
reverseVideo = "\^[[7m"
resetVideo = "\^[[0m"

clearScreen :: IO ()
clearScreen = Text.putStr clearTerm
...
formatFileInfo FileInfo{..} maxWidth totalPages currentPage =
  ...
  where
    ...
    truncateStatusLine t
      | maxWidth <= 3 = ""
      | Text.length t > maxWidth =
          Text.take (maxWidth - 3) t <> "..."
      | otherwise = t
...
runHCat = do
  ...
  contents <- Text.readFile targetFilePath
...
wordWrap lineLength lineText =
  case Text.splitAt lineLength lineText of
    (fullLine, "") -> [fullLine]
  ...
  where
    softWrap hardWrapped =
      let (rest, wrappedText) = Text.break isSpace $ Text.reverse hardWrapped
       in (Text.reverse wrappedText, Text.reverse rest)
